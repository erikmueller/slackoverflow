...
Other languages like [Elixir](http://elixir-lang.org/){target="blank"} (which we're going to talk about in a bit) make it much easier to use functional language features and methods such as _destructuring_, _pattern matching_ or _recursion_.
...
This first part will take a look at how **Elixir** might help you to rethink programming patterns leading to cleaner code and continue with what this [**Phoenix**](http://www.phoenixframework.org/){target="blank"} thing actually is.
...
Elixir runs on the [Erlang](https://www.erlang.org/){target="blank"} VM (BEAM) and gives you the great ecosystem companies like _Heroku_ and _WhatsApp_ use to build their products.
...
Elixir's syntax is concise and reminds of the Ruby programming language due to the fact that the creator [José Valim](https://github.com/josevalim){target="blank"} is a very active member of the ruby community.
...
A good place to start is [the official elixir-lang site](http://elixir-lang.org/){target="blank"}.
...
Take a look at [Scala](https://www.scala-lang.org/){target="blank"}, [Haskell](https://www.haskell.org/){target="blank"} or Erlang itself.
